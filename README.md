...
> **Tip:** The generator script (`scripts/gen-textures.go`) uses only Go's standard
> library — no third-party packages needed.
...
│   └── gen-textures.go         # Regenerates stock cat textures + helloworld block texture
├── go.mod
├── package.json
└── .gitignore
```
...
## helloworld custom block

The add-on includes a custom block called **`hello:helloworld`**.

- **Behavior:** identical to dirt — 0.5 s hand-mine time, grass dig sounds, drops itself.
- **Texture:** 16 × 16 green block with pixel-art "HW" letters (generated by `gen-textures.go`).
- **Availability:** creative inventory only (Construction tab). The block does not generate
  naturally, so survival players cannot obtain it without commands.
- **Give command:** `/give @s hello:helloworld`
